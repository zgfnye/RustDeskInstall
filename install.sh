...
RDLATEST=$(curl https://gh-proxy.zgfnye.ggff.net/https://api.github.com/repos/rustdesk/rustdesk-server/releases/latest -s | grep "tag_name" | awk -F'"' '{print $4}')

echo "Installing Rustdesk Server"
if [ "${ARCH}" = "x86_64" ] ; then
wget "https://gh-proxy.zgfnye.ggff.net/https://github.com/rustdesk/rustdesk-server/releases/download/${RDLATEST}/rustdesk-server-linux-amd64.zip"
unzip rustdesk-server-linux-amd64.zip
mv amd64/* /opt/rustdesk/
elif [ "${ARCH}" = "armv7l" ] ; then
wget "https://gh-proxy.zgfnye.ggff.net/https://github.com/rustdesk/rustdesk-server/releases/download/${RDLATEST}/rustdesk-server-linux-armv7.zip"
unzip rustdesk-server-linux-armv7.zip
mv armv7/* /opt/rustdesk/
elif [ "${ARCH}" = "aarch64" ] ; then
wget "https://gh-proxy.zgfnye.ggff.net/https://github.com/rustdesk/rustdesk-server/releases/download/${RDLATEST}/rustdesk-server-linux-arm64v8.zip"
...
function setuphttp () {
    ...
    wget https://gh-proxy.zgfnye.ggff.net/https://raw.githubusercontent.com/dinger1986/rustdeskinstall/master/WindowsAgentAIOInstall.ps1
    sudo sed -i "s|secure-string|${string64rev}|g" WindowsAgentAIOInstall.ps1

    # Create linux install script
    wget https://gh-proxy.zgfnye.ggff.net/https://raw.githubusercontent.com/dinger1986/rustdeskinstall/master/linuxclientinstall.sh
    ...
    cd /opt/gohttp
    GOHTTPLATEST=$(curl https://gh-proxy.zgfnye.ggff.net/https://api.github.com/repos/codeskyblue/gohttpserver/releases/latest -s | grep "tag_name"| awk '{print substr($2, 2, length($2)-3) }')

    echo "Installing Go HTTP Server"
    if [ "${ARCH}" = "x86_64" ] ; then
    wget "https://gh-proxy.zgfnye.ggff.net/https://github.com/codeskyblue/gohttpserver/releases/download/${GOHTTPLATEST}/gohttpserver_${GOHTTPLATEST}_linux_amd64.tar.gz"
    tar -xf  gohttpserver_${GOHTTPLATEST}_linux_amd64.tar.gz 
    elif [ "${ARCH}" =  "aarch64" ] ; then
    wget "https://gh-proxy.zgfnye.ggff.net/https://github.com/codeskyblue/gohttpserver/releases/download/${GOHTTPLATEST}/gohttpserver_${GOHTTPLATEST}_linux_arm64.tar.gz"
...
if [[ -z "$http" ]]; then
    ...
    echo - e "You can get a free API with Addressbook etc via https://gh-proxy.zgfnye.ggff.net/https://github.com/infiniteremote/installer"
